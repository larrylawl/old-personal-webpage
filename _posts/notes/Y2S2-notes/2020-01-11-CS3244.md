...
Consider target concept \$ c : X \rarrow T \$ where each instance \$ x \in X \$ is represented by input attributes \$ x = (x_1, \cdots, c_n)^T \$. 
...
Using **naive Bayes assumption** \$ P(x_1, \cdots, x_n|t) = \prod^n_{i = 1}P(x_i \mid t) \$, 

$$
t_{\mathrm{NB}}=\underset{t \in T}{\arg \max } P(t) \prod_{i=1}^{n} P\left(x_{i} | t\right)
$$