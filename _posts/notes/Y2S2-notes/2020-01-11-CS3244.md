...
2. \$ P(D \mid h) \$: probability of observing data D given some world in which hypothesis *h* holds.
> E.g. P(D) is the probability of the target value \$ t_d \in D \$?
1. \$ P(D) = \sum_{h \in H} P(D \mid h)P(h) \$: prior probability that training data D will be observed (ie the probability of D given no knowledge about which hypothesis holds)
> Law of total probability
4. \$ P(h \mid D) \$: posterior belief of h given D
...
The converse is not true. Proof by counter example. Suppose there exist a hypothesis \$ h \in H \$ such that \$ P(D|h) = 1\$ and \$ P(h) = 0.95 \$. \$ h_{MAP} = h_{ML} \$. However, there can still exist another \$ h' \in H \$ such that \$ P(h') = 0.05 \$. Thus \$ P(h) \neq P(h') \$. 
...
> \$ h_{ML} is the probability of an output

> \$ P(C(x_d) = 1) = P(t_D = 1|x_d) \$ is the true probability of the class label \$ c(x_d) \$ of value1 given input instance \$ x_d \$
...
1. \$ -log_2P(h) \$ is the description length of h under optimal code for H
2. \$ -log_2P(D|h) \$ is the description legnth of D given h under optimal code for describing data D
