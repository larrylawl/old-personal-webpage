...
**Problem.** What is some examples *x* are missing values of attribute *A*?
...
3. Assign *probability* \$ p_i \$ to each possible values of *A*.
...
2. If t = +1 and o = -1, \$ (t - o) = 2 \implies \Delta = 2\eta x_i \implies \$ weights will be increased. This align with the objective of making \$ t = o \$: for \$ o = 1 \$, \$ \overrightarrow(w) \cdot \overrightarrow(x) > 0 \$, thus weights need to be increased.
...
**Batch Gradient Descent.** Over a batch of training examples rather than all the trianing examples.