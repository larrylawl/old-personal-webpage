...
- [Greedy Algorithms](#greedy-algorithms)
  ...
  - [Prefix Code and Lebelled Binary Tree](#prefix-code-and-lebelled-binary-tree)
...
<<(\lg n) ! \equiv n\lg\lg n <<n^{\lg n}<<2^{n}-n^{2} << (\lg n)^n << n!
$$
...
## Universal Hashing
**Motivation.** Any fixed hash function is vulnerable to an adversary choosing *n* keys that all hash to the same slot, yielding an average retrieval time of \$ \Theta(n) \$. Need the hash function to *randomly* choose in a way that is *independent* of the keys.
...
2. Show that bank balance never go negative. (ie...)
...
**Cut-and-paste** argument is often used to prove optimal substructure. 
1. In particular, you want to show that when you come up with an optimal solution to a problem, you have necessarily used optimal solutions to the constituent subproblems. 
2. The proof is by contradiction. 
3. Suppose you came up with an optimal solution to a problem by using suboptimal solutions to subproblems. 
4. Then, if you were to replace ("cut") those suboptimal subproblem solutions with optimal subproblem solutions (by "pasting" them in), you would improve your optimal solution. 
5. But, since your solution was optimal by assumption, you have a contradiction.

In the example,
1. The recurrence relation is the optimal substructure.
2. Suppose M[j] = t
3. Replace M[j'] < t - 1 with M[j'] = t
4. M[j] < t (since \$ d_{i_t} \$ is in the supposed optimal solution). Contradiction.


> Q: How does cut-and-paste prove optimal substructure?
...
**Top-Down** approach solves the recurrence only when needed to **Bottom-Up** (eg case of Fibonacci numbers) solves all recurrences.
...
1. Cast the problem where have to **make a choice and are left with one subproblem** to solve.
...
**Greedy Optimum Substructure**
![OS](/assets/img/2020-16-1-CS3230/greedy-os-eg.png)

## Prefix Code and Lebelled Binary Tree
...
**Theorem:** If \$ X \subseteq V \$ is a vertex cover of *G* **iff** \$ V\backslashX \$ is an independent set of *G*
