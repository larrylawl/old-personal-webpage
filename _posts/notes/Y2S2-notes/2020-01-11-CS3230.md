...
1. n! permutations => Decision tree must contain ≥ n! leaves
...
1. \$ n + 2^r \$: each *b*-bit word is broken into *r*-bit pieces
...
$$
T(\text{Radix Sort}) = T(n, b) = \Theta(bn/lgn)
$$

1. For numbers in the range from 0 to \$ n^d - 1 \$, we have \$ b = dlgn \implies \Theta(dn) \$
> *b* denotes the number of bits needed to represent number \$ n^d - 1 \$.
